...
- `straddle customers update` — Updates an existing customer's information. This endpoint allows you to modify the customer's contact details, PII,... Pass `--status` explicitly, or include `status` in the `--stdin` JSON body, even with `--dry-run`. Read the existing customer first when you intend to preserve its status.
