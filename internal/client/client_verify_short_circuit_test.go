...
// short-circuit. Under STRADDLE_VERIFY=1 with no
// STRADDLE_VERIFY_LIVE_HTTP opt-in, every mutating verb
// (DELETE/POST/PUT/PATCH) must return a synthetic envelope without
// dialing.
//
// A future client edit that drops the gate, narrows the verb list, or
// removes either env-var check would silently re-open the
// agent-readiness gap the gate was added to close.
func TestClient_VerifyShortCircuit_MutatingVerbs(t *testing.T) {
	for _, verb := range []string{"DELETE", "POST", "PUT", "PATCH"} {
		...
		t.Run(verb, func(t *testing.T) {
			...
			// STRADDLE_VERIFY_LIVE_HTTP must NOT be set for the short-circuit
			// branch. Explicit unset defends against a stale value in the parent env.
			...
			if got, _ := env["__straddle_verify_synthetic__"].(bool); !got {
				t.Fatalf("envelope must include __straddle_verify_synthetic__: true; got %v", env)
			}
...
// contract: when STRADDLE_VERIFY_LIVE_HTTP=1 is set alongside
// STRADDLE_VERIFY=1, the short-circuit does NOT fire and the transport
// dials. The verifier's httptest mock server depends on this opt-in path
// to receive mutating requests so its pass/fail assertions can run.
func TestClient_VerifyShortCircuit_LiveHTTPOptIn(t *testing.T) {
	t.Setenv("STRADDLE_VERIFY", "1")
	t.Setenv("STRADDLE_VERIFY_LIVE_HTTP", "1")
	c, rec := newClientWithRecorder(t)

	_, _, _ = c.do("DELETE", "/test", nil, nil, nil)

	if rec.calls < 1 {
		t.Fatalf("STRADDLE_VERIFY_LIVE_HTTP=1 should opt back in to real dial; recorder saw %d calls", rec.calls)
	}
}
...
// STRADDLE_VERIFY=1, because they cannot mutate remote state. A
// regression that broadens isMutatingVerb to include GET would break
...
// under STRADDLE_VERIFY=1, because the operation does not mutate remote
// state. Without this, an inherited verify env silently breaks every read
// on a shared-endpoint API.
