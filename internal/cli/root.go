...
type rootFlags struct {
	...
	// straddleAccount holds --account: the embedded account a platform call
	// acts on behalf of, sent as the Straddle-Account-Id header. See
	// straddle_account.go.
	straddleAccount string
	// straddleAccountResolved is the account value resolved in the pre-run
	// (flag or sticky, after integration-type gating); newClient applies it.
	straddleAccountResolved string
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// without a straddle:endpoint annotation.
		currentResource = resourceFromEndpoint(cmd.Annotations["straddle:endpoint"])
		return nil
	}
	...
	rootCmd.AddCommand(newPipelineCmd(flags))
	rootCmd.AddCommand(newReturnsCmd(flags))
	rootCmd.AddCommand(newReviewQueueCmd(flags))
