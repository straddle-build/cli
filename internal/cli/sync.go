...
Resource scoping:
  ...
  straddle sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Under STRADDLE_VERIFY=1 (mock/dry-run), all goroutines reach
			// SQLite without the natural serialization that network latency
			// provides in real syncs, so the worker pool races on the writer
			// and trips SQLITE_BUSY despite _busy_timeout.
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "capability_requests", ParentTable: "accounts", ParentIDParam: "account_id", PathTemplate: "/v1/accounts/{account_id}/capability_requests", KeyField: ""},
	}
}
