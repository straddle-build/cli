...
// first leaf that (a) carries the `straddle:endpoint` annotation, so it
// actually dials the API rather than reading a local file like `feedback list` or
...
func suggestReadCommand(root *cobra.Command) string {
	...
	walk = func(cmd *cobra.Command, path []string) {
		...
		for _, child := range cmd.Commands() {
			...
			// Recurse even into Hidden parents: the CLI marks raw resource
			// parents Hidden to keep --help curated, but their endpoint leaves
			// remain runnable (`<cli> projects list` works). Skipping hidden
			// subtrees would make this return "" in nearly every CLI.
			// isSuggestableReadLeaf still rejects a leaf that is itself Hidden.
...
func isSuggestableReadLeaf(cmd *cobra.Command) bool {
	...
	// supposed to avoid.
	if cmd.Annotations["straddle:endpoint"] == "" {
		return false
	}
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  straddle doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("STRADDLE_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "STRADDLE_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "STRADDLE_API_KEY")
			}
			switch {
			...
			// inherits STRADDLE_VERIFY=1 (parent shell, CI runner, container
			...
			if cliutil.IsVerifyEnv() {
				if cliutil.IsVerifyLiveHTTPEnv() {
					...
					report["verify_mode"] = "INFO ACTIVE — mutating HTTP verbs short-circuit (STRADDLE_VERIFY=1; no network calls for DELETE/POST/PUT/PATCH)"
				}
			} else {
