...
- `sql`: run one read-only `SELECT` or `WITH ... SELECT` statement against the local SQLite database. Semicolons inside SQLite literals, quoted identifiers, and comments do not split the statement.
